...
PoseEstimator::PoseEstimator(pcl::Registration<pcl::PointXYZI, pcl::PointXYZI>::Ptr& registration, const Eigen::Vector3f& pose, const Eigen::Quaternionf& quat, double time_duration)
  : registration(registration), time_duration(time_duration) 
{
    ...
    last_observation.block<3, 1>(0, 3) = pose;
    ...
    PoseSystem system;
    ukf.reset(new sentry_localization::UnscentedKalmanFilterX<float, PoseSystem>(system, 16, 6, 7, process_noise, measurement_noise, mean, cov));
}
...
{
  if(initial_stamp.is_zero())
  {
    initial_stamp = stamp;
  }

  if((stamp - initial_stamp).toSec() < time_duration || prev_stamp.is_zero() || prev_stamp == stamp){
    prev_stamp = stamp;
    return;
  }
...
void PoseEstimator::predict_odom(const Eigen::Matrix4f& odom_delta)
{
  if(!odom_ukf)
  {
    ...
    odom_ukf.reset(new sentry_localization::UnscentedKalmanFilterX<float, OdomSystem>(odom_system, 7, 7, 7, odom_process_noise, odom_measurement_noise, odom_mean, odom_cov));
  }
...
pcl::PointCloud<pcl::PointXYZI>::Ptr PoseEstimator::correct(const ros::Time& stamp, const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& cloud)
{
  if(initial_stamp.is_zero())
  {
    initial_stamp = stamp;
...
Eigen::Vector3f PoseEstimator::pose() const
{
...
Eigen::Vector3f PoseEstimator::odom_pose() const
{
