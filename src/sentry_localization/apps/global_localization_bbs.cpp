...
#include <include/global_localization/bbs/bbs_localization.hpp>
#include <include/global_localization/bbs/occupancy_gridmap.hpp>
...
GlobalLocalizationBBS::GlobalLocalizationBBS(ros::NodeHandle& private_nh) : private_nh(private_nh) {
  pub_gridmap = private_nh.advertise<nav_msgs::OccupancyGrid>("bbs/gridmap", 1, true);
  pub_map_slice = private_nh.advertise<sensor_msgs::PointCloud2>("bbs/map_slice", 1, true);
  pub_scan_slice = private_nh.advertise<sensor_msgs::PointCloud2>("bbs/scan_slice", 1, false);
}
...
void GlobalLocalizationBBS::set_global_map(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud) {
  ...
  pub_map_slice.publish(map_3d);
  pub_gridmap.publish(bbs->gridmap()->to_rosmsg());
}
...
GlobalLocalizationResults GlobalLocalizationBBS::query(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, int max_num_candidates) {
  ...
  if (pub_scan_slice.getNumSubscribers()) {
    auto scan_3d = unslice(scan_2d);
    scan_3d->header = cloud->header;
    pub_scan_slice.publish(scan_3d);
  }
